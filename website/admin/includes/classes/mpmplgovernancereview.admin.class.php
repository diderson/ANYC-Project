...
class Mpmplgovernancereview extends EntityManager {
	...
	function __construct( $id = 0, $templateType = 'list', $pageSize = 10, $currentPage = 1, $sortBy = '' ) {
		...
		$this->tableName = $this->tablePrefix.'mp_mpl_govreview_ass';
		...
		//Edit page variables		
		$this->editFileName = 'mpmplgorvernancereview-edit';										
		$this->editHtmlPageTitle = 'Councillor Governance review';
		$this->editPageTitle = 'Councillor Governance review';
		$this->editContentTemplate = 'mpmplgorvernancereview-edit.tpl';
		$this->insertHtmlPageTitle = 'Insert Administrator';
		$this->insertPageTitle = 'Councillor Governance review';
		$this->editPanelHeading	= 'ASSESSMENT: TO BE COMPLETED BY THE COUNCILLOR (Compulsory)';	
	...
	function getEditData( &$recordSet ) {		
		...
		$sql = "SELECT DISTINCT
					".mysql_escape_string( $this->tableName) .".id_mp_mpl_govreview_ass as 'id_mp_mpl_govreview_ass',
					".mysql_escape_string( $this->tableName) .".id_administrator as 'id',
					".mysql_escape_string( $this->tableName) .".mp_mpl_govreview as 'mp_mpl_govreview_json',
					".mysql_escape_string( $this->tableName) .".mp_mpl_govpr_review as 'mp_mpl_govpr_review_json',
					".mysql_escape_string( $this->tableName) .".mp_mpl_constreview as 'mp_mpl_constreview_json',
					".mysql_escape_string( $this->tableName) .".mp_mpl_status as 'mp_mpl_status',
					".mysql_escape_string( $this->tableName) .".cdate as 'cdate',
