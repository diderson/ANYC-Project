...
class Provsecgovernancereview extends EntityManager {
  ...
  function __construct( $id = 0, $templateType = 'list', $pageSize = 10, $currentPage = 1, $sortBy = '' ) {
    ...
    $this->tableName = $this->tablePrefix.'provsec_preview_ass';
    ...
    //List page variables
    $this->listFileName = 'provsecreview-list';   
    $this->listContentTemplate = 'provsecreview-list.tpl';
    $this->listHtmlPageTitle = 'Regional Secretary Governance review';
    $this->listPageTitle ='Regional Secretary Governance review';
    ...
    //Edit page variables   
    $this->editFileName = 'provsecreview-edit';                    
    $this->editHtmlPageTitle = 'Regional Secretary Governance review';
    $this->editPageTitle = 'Regional Secretary Governance review';
    $this->editContentTemplate = 'provsecreview-edit.tpl';
    $this->insertHtmlPageTitle = 'Regional Secretary Governance review';
    $this->insertPageTitle = 'Regional Secretary Governance review';
    $this->editPanelHeading = 'To be Completed by the Regional Secretary (Compulsory)'; 
  ...
  function getEditData( &$recordSet ) {   
    ...
    $sql = "SELECT DISTINCT 
          ".mysql_escape_string( $this->tableName) .".id_provsec_preview_ass as 'id_provsec_preview_ass',
          ...
          ".mysql_escape_string( $this->tableName) .".provsec_preview as 'provsec_preview_json',
          ".mysql_escape_string( $this->tableName) .".provsec_ovpefrating as 'provsec_ovpefrating_json',
    ...
    if ($querySuccess && sizeof($recordSet) > 0) {
      if ($recordSet[0]['provsec_preview_json'] !='') $recordSet[0]['provsec_preview'] = json_decode($recordSet[0]['provsec_preview_json'], true);
      if ($recordSet[0]['provsec_ovpefrating_json'] !='') $recordSet[0]['provsec_ovpefrating'] = json_decode($recordSet[0]['provsec_ovpefrating_json'], true);
  ...
  function getCouncillorReview($id_mp_mpl_govreview_ass, &$recordSet){
     ...
     $sql = "SELECT DISTINCT
            ...
            AND anc_profile.id_municipality = ".$_SESSION['BACKEND_ADMIN']['ADMIN_MUNICIPALITY'];
  ...
  /************************************************/
  /************************************************/
  function getProsecGovernanceReview ( &$recordSet) {
    $sql = "SELECT DISTINCT
            anc_provsec_preview.id_provsec_preview as 'id_provsec_preview',
            anc_provsec_preview.provsec_preview_libele as 'provsec_preview_libele'
          FROM 
            anc_provsec_preview 
          ORDER BY anc_provsec_preview.id_provsec_preview";
