...
class Administrator extends EntityManager {
	...
	function __construct( $id = 0, $templateType = 'list', $pageSize = 10, $currentPage = 1, $sortBy = '' ) {
		...
		//Edit page variables		
		$this->editFileName = 'administrator-edit';										
		$this->editHtmlPageTitle = 'Edit Administrator';
		$this->editPageTitle = 'Edit Administrator';
		$this->editContentTemplate = 'administrator-edit.tpl';
		$this->insertHtmlPageTitle = 'Insert Administrator';
		$this->insertPageTitle = 'Insert Administrator';
		$this->editPanelHeading	= 'Administrator Details';	
			...
			if( strlen( $password ) <= 32 ) {
				$sql = 
					"SELECT  
						...
						".mysql_escape_string( $this->tableName) .".email_address as 'email_address',
						".mysql_escape_string( $this->tableName) .".password as 'password',
						".mysql_escape_string( $this->tableName) .".administrator_profile_status as 'administrator_profile_status',
				...
				if( $querySuccess && sizeof( $recordSet ) > 0 && strcmp ( $recordSet[0]['password'] , $this->encrypt( mysql_escape_string( $password ) ) ) == 0  ) {					
					...
					if( $recordSet[0]['id_administrator'] != '' 
						...
						$this->administrator_profile_status = $recordSet[0]['administrator_profile_status'];
						$this->ldate = $recordSet[0]['ldate'];
						$this->administratorType = $recordSet[0]['administratortype_alias'];
						$this->id_municipality = $recordSet[0]['id_municipality'];						
	...
	function setSessionVariables() {
		...
		$_SESSION[$this->backend_ID]['ADMIN_PROFILESTATUS'] = $this->administrator_profile_status;
		$_SESSION[$this->backend_ID]['ADMIN_LASTLOGINDATE'] = $this->ldate;
		$_SESSION[$this->backend_ID]['ADMIN_TYPE'] = $this->administratorType;	
		$_SESSION[$this->backend_ID]['ADMIN_MUNICIPALITY'] = $this->id_municipality;		
