...
if ($_SESSION['BACKEND_ADMIN']['ADMIN_TYPE'] != 'chiefwhip') {
...
// Edit per file
/******************************************************************************/
$pageObject = new Cwgovernancereview( $id, 'edit' );
...
$pageObject->getCwGovernanceReview ($cwGovernanceData);
$smarty->assign( 'cwGovernanceData', $cwGovernanceData );
...
if (sizeof($editCouncillorAssData) == 0) {
  header( "Location: /admin/cwgovernancereview-list" ); 
}
...
if( $formAction != '' ) {
 // $pageObject->id = $_SESSION['BACKEND_ADMIN']['ADMIN_ID'];
  $redirect = false;
  $preview = array();
  $cw_preview_size = sizeof($_POST['cw_preview_size']) + 1;

  for ($i=1; $cw_preview_size > $i; $i++) {
    $preview['preview_'.$i] = $_POST['preview_'.$i] ;
  }
  
  $_POST['cw_preview'] = json_encode($preview);
  ...
  $_POST['cw_ovpefrating'] = json_encode($ovpefrating);
