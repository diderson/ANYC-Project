...
/*
 ...
 | phpMyBackupPro                                                           |
 +--------------------------------------------------------------------------+
 | Copyright (c) 2004-2011 by Dirk Randhahn                                 |                               
...
foreach($CONF as $key=>$value) {
    if (array_key_exists($key,$validate)) {
        switch($validate[$key]) {
            case "int": if (!eregi("^[0-9]*$",$value)) echo PMBP_addOutput(array_search($key,$conf)."' ".C_WRONG_TYPE,"red");
                break;
            case "float": if (!eregi("^[0-9]*\.?[0-9]*$",$value)) echo PMBP_addOutput(array_search($key,$conf)."' ".C_WRONG_TYPE,"red");
                break;
